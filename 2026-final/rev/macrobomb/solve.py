flag_len = 45
main = open("main.c").readlines()
flag = main[3]
flag = flag[len("char flag[46] = "):-2]
flag = eval("b" + flag)
print(f"{flag = }")
...
fuck_flag = bytearray(map(lambda x: x%256, sol))
for i in range(len(fuck_flag)):
    fuck_flag[i] = (flag[i] - fuck_flag[i]) % 256
